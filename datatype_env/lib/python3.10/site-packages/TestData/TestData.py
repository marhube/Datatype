...
def create_datetime_column(n: int, with_na: bool) -> pd.Series:
    ...
    col = pd.Series(pd.date_range(start='2023-01-01', periods=n, freq='h'))
    if with_na:
        col[::4] = pd.NaT
    return col

def create_nat_column(n:int,dtype: str = 'datetime64[ns]') -> pd.Series:
    
    col = pd.Series([pd.NaT for iter in range(n)],dtype = dtype)
    return col
